...
public class AskSourceSelector extends SourceSelectorWrapper implements SourceSelector
{
	...
	*/
	...
	private boolean askPattern(StatementPattern pattern, URL source, boolean allow_select ) throws IOException
	{
		...
		try (InputStream input = new FileInputStream(System.getProperty("user.dir")+"/config.properties")) {
			...
			prop.load(input);
			ss_file = prop.getProperty("ss.file");
		}
		...
		if( conn!= null ) {
			try {
				...
				logger.warn( "Failed to contact source {s} to ASK about pattern {}. Exception: {}",
						source, pattern.toString(), ex.getMessage() );
			}
		}
		...
		if( allow_select && (conn!=null) ) {
			...
			try {
				...
				if (retv) {					
					try(BufferedWriter ss_writer = new BufferedWriter(new FileWriter(new File(ss_file), true))) {
						ss_writer.write(String.valueOf(pattern.toString()+";"+source+"\n"));
					}
				}
			}
				...
				logger.warn( "Failed to contact source {} to execute query {}. Exception: {}",
						source, q.toString(), ex.getMessage() );
			}
